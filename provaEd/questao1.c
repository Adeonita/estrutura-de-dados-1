...
void separaNumero(Lista *inicio_da_lista){
    Nodo *atual = inicio_da_lista->primeiro_no;

    while(atual != NULL){
        if(atual->info > 0){
            printf("%d será inserido numa pilha", atual->info);
        }else{
            printf("%d será inserido numa fila", atual->info);
        }
        atual = atual->prox;
    }
}
...
int main(){ 
    ...
    if(lista){
        while(opcao != 0){
            printf("Digite 1 para inserir, 0 para sair, 2 para exibir, 3 para checar, 4 para retirar, 5 para seprar: ");
            scanf("%d",&opcao);
            if(opcao == 1){
                int n;
